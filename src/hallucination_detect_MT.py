...
MULTITHREADING VERSION - Uses ThreadPoolExecutor for concurrent processing
...
Version: 1.0 - Multithreading Edition
"""
...
# ===================== CONFIG =====================
INPUT_FILE = "hallucinationQuestions_all.parquet"
OUTPUT_FILE = "hallucination_detection_results.csv"
DOCUMENTATION_FILE = "Openbank_extracted_text.txt"
PROMPT_FILE = "hallucination_evaluation_prompt.txt"  # Evaluation prompt template file
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("hallucination_detection_MT.log")
    ]
)
log = logging.getLogger("hallucination_detector_MT")
...
def evaluate_one_question_threaded(
    idx: int,
    question: str,
    response: str,
    context: str,
    ...
    """
    Worker function for multithreading - evaluate a single question/answer pair for hallucinations.

    This function runs in a worker thread and uses a shared WatsonX client and TokenManager.
    ...
    Args:
        ...
        watsonx: Shared WatsonX client instance
        token_manager: Shared TokenManager instance
...
def generate_hallucination_report(df: pd.DataFrame, output_dir: str = "hallucination_reports"):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write("Hallucination Detection Results (MULTITHREADING)\n")
        f.write("="*100 + "\n\n")
        ...
        f.write(f"Processing Mode: MULTITHREADING ({MAX_WORKERS} threads)\n")
...
def generate_excel_report(df: pd.DataFrame, output_dir: str = "hallucination_reports"):
    ...
    excel_file = os.path.join(output_dir, f"HALLUCINATION_REPORT_DETAILED_MT_{timestamp}.xlsx")
...
def main():
    ...
    print(f"Processing mode: {'MULTITHREADING' if USE_PARALLEL else 'SEQUENTIAL'}")
    if USE_PARALLEL:
    ...
    # Initialize shared WatsonX client and TokenManager
    from watsonx import WatsonX
    watsonx = WatsonX()
    oauth_url = f"{BASE_URL_WATSONX}/oauth2/accesstoken-clientcredentials"
    token_manager = TokenManager(watsonx, oauth_url, BASIC_CREDENTIALS_WATSONX)

    # Lock for DataFrame updates (thread-safe writes)
    df_lock = Lock()
    ...
    try:
        if USE_PARALLEL:
            log.info(f"Starting MULTITHREADING evaluation with {MAX_WORKERS} threads")

            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                # Submit all tasks
                futures = {
                    executor.submit(
                        evaluate_one_question_threaded,
                        idx,
                        str(row["questions"]),
                        str(row["answer"]),
                        str(row.get("context", "")),
            ...
            # Sequential mode
            log.info(f"Starting SEQUENTIAL hallucination detection")
            for idx, row in pending.iterrows():
                try:
                    (
                        _,
                    ...
                    df_eval.at[idx, "unsupported_claims"] = unsupported_claims
                    df_eval.at[idx, "contradicted_claims"] = contradicted_claims
                    df_eval.at[idx, "fabricated_details"] = fabricated_details
                    df_eval.at[idx, "supported_claims"] = supported_claims
                    df_eval.at[idx, "reasoning"] = reasoning
                    df_eval.at[idx, "raw_llm_response"] = raw_response

                    halluc_status = "HALLUCINATION" if hallucination_detected else "PASS"
                    log.info(f"Row {idx}: {verdict} ({severity}) - {halluc_status}")
    ...
    # Console summary
    print("\n" + "="*100)
    print("HALLUCINATION DETECTION COMPLETE (MULTITHREADING)")
    print("="*100)
    ...
    print(f"  4. Execution Log: hallucination_detection_MT.log")
