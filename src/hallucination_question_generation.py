...
# Generation settings
TARGET_COUNT = 500   # Number of hallucination questions to generate
SAMPLE_SIZE = 100    # Number of base questions to sample from
...
def _process_base_question(args):
    ...
    Args:
        args: Tuple of (index, base_question, questions_per_base, context_text, provider, model, api_key)

    Returns:
        Tuple of (index, list of generated questions)
    """
    index, base_question, questions_per_base, context_text, provider, model, api_key = args
...
Return ONLY the questions, one per line, no numbering or extra text."""
    ...
    response_text = generate_with_llm(prompt, provider, model, api_key)
...
def generate_hallucination_questions(target_count: int = None, sample_size: int = None,
    ...
    for i, base_question in enumerate(sampled_questions):
        ...
        worker_args.append((i, base_question, questions_per_base, context_text, provider, model, api_key))
